...
for p in paczki:
    try:
        ...
        print("elf", id, "EXCEPTION OCCURED")
        conn.rollback()

avg = sum(times)/len(times) if len(times)>0 else 0
print(f'elf-{id},ile_paczek,{len(paczki)},ile_ok,{counter},ile_zle,{len(paczki) - counter},avg_t,{avg}')
